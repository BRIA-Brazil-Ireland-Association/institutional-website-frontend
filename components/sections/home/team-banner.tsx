...
export function TeamBanner({ content }: any) {
  const banner = getObject(content, "banner");
  const title = getText(banner, "title");
  const sectionTitle = getText(banner, "sectionTitle");
  const description = getText(banner, "description");
  const image = getObject(banner, "image");
  const imageUrl = getMediaUrl(image);
  const imageAlt = getText(image, "alternativeText") ?? "";
  const imageWidth = typeof image?.width === "number" ? image.width : 1484;
  const imageHeight = typeof image?.height === "number" ? image.height : 698;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ctas: any[] = Array.isArray(banner?.cta) ? banner.cta : [];
