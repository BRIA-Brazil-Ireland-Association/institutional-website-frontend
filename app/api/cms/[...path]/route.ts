import { proxyCmsGet, proxyCmsPost } from "@/services/cms";
...
export async function POST(
  ...
  try {
    ...
    const cmsResponse = await proxyCmsPost({ body, path });
    const responseHeaders = new Headers({
      "Cache-Control": "no-store",
    });
    const contentType = cmsResponse.headers.get("content-type");
    ...
    const message =
      error instanceof Error ? error.message : "Failed to submit CMS content.";
    const status = message.includes("STRAPI_CMS_URL") ? 500 : 502;
